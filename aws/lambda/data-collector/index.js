...
async function writeToS3(snapshot) {
  ...
  for (const colony of snapshot.colonies || []) {
    const flatRecord = {
      ...
      totalCreeps: colony.creeps?.total || 0,
      harvester: colony.creeps?.byRole?.HARVESTER || 0,
      hauler: colony.creeps?.byRole?.HAULER || 0,
      upgrader: colony.creeps?.byRole?.UPGRADER || 0,
      builder: colony.creeps?.byRole?.BUILDER || 0,
      defender: colony.creeps?.byRole?.DEFENDER || 0,
      remoteMiner: colony.creeps?.byRole?.REMOTE_MINER || 0,
      remoteHauler: colony.creeps?.byRole?.REMOTE_HAULER || 0,
      reserver: colony.creeps?.byRole?.RESERVER || 0,
