...
import { blocksMovement, isChokepoint } from "./buildGrid";
/**
...
function isValidBuildPos(room: Room, x: number, y: number, terrain: RoomTerrain): boolean {
  ...
  // Corridor guard, shared with ExtensionPlanner via buildGrid. Keeping it private here
  // is how that planner came to place an extension in E47N41's only route north.
  const blocked = (nx: number, ny: number): boolean => {
    if (terrain.get(nx, ny) === TERRAIN_MASK_WALL) return true;
    for (const st of room.lookForAt(LOOK_STRUCTURES, nx, ny)) {
      if (blocksMovement(st.structureType)) return true;
    }
    return false;
  };
  if (isChokepoint(x, y, blocked)) return false;
