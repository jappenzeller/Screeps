...
/**
 * A target container holding less than this cannot meaningfully finish a partial load.
 *
 * The release below used to require the container to be exactly empty. A static miner sits
 * on its container and transfers as it harvests, so the container holds a small constant
 * amount - 10, in both of E46N37's - and `=== 0` was therefore never true. Two haulers sat
 * in COLLECTING holding 873 and 450 energy while 27 extensions were empty, storage was 0
 * and the room ran at 49.7% of capacity, each waiting on 10/tick to top off a load it
 * already had. The comment said "drained"; the test said zero, which is a value that never
 * occurs. A threshold measures what was meant.
 */
const TOPUP_WORTH_WAITING = 100;
...
export function runHauler(creep: Creep): void {
  ...
  if (
    ...
    if (!src || src.store[RESOURCE_ENERGY] < TOPUP_WORTH_WAITING) {
      creep.memory.state = "DELIVERING";
      delete creep.memory.targetContainer;
      creep.say("DLV");
    }
